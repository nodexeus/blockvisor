...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::List { protocol, detailed, client, network } => {
            handle_list_command(protocol, detailed, client, network).await
        },
        Commands::Download { 
            protocol, client, network, output, node_type, version, 
            workers, max_connections, dry_run 
        } => {
            handle_download_command(
                protocol, client, network, output, node_type, 
                version, workers, max_connections, dry_run
            ).await
        },
...
fn print_download_info(info: &DownloadInfo) -> Result<()> {
    println!("Would download: {}/{}/{} version {}", 
    ...
    println!("Estimated time: {:.0}-{:.0} minutes", estimated_minutes * 0.5, estimated_minutes * 2.0);
...
fn print_status(status: &DownloadStatus) -> Result<()> {
    match status {
        ...
        DownloadStatus::InProgress { 
            ...
            println!("Progress: {}% complete ({:.1}GB/{:.1}GB downloaded)", 
                progress_percent,
