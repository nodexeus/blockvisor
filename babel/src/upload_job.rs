...
impl<C: BabelEngineConnector + Send> Runner for Uploader<C> {
    async fn run(&mut self, mut run: RunFlag) -> Result<()> {
        let parts_path = self.config.archive_jobs_meta_dir.join(BLUEPRINT_FILENAME);
        let chunks_path = self.config.archive_jobs_meta_dir.join(CHUNKS_FILENAME);
        let (mut parts, uploaded) = if let Ok(parts) = load_job_data::<UploadParts>(&parts_path) {
            (parts, load_chunks(&chunks_path)?)
        } else {
            let manifest = self.prepare_blueprint()?;
            ...
            save_job_data(&parts_path, &parts)?;
            assign_slots(&mut parts.manifest.chunks, slots.slots);
        ...
        for chunk in &mut parts.manifest.chunks {
        ...
        let custom_timeout = RPC_REQUEST_TIMEOUT
            + bv_utils::rpc::estimate_put_download_manifest_request_timeout(
                parts.manifest.chunks.len(),
            );
        let mut client = self.connector.connect();
        client
            .put_download_manifest(with_timeout(parts.manifest, custom_timeout))
            .await
...
fn assign_slots(chunks: &mut [Chunk], slots: Vec<Slot>) {
    let mut chunks_to_update = chunks.iter_mut().rev().take(slots.len());
    for slot in slots {
        if let Some(chunk) = chunks_to_update.find(|chunk| chunk.index == slot.index) {
            chunk.key = slot.key;
...
//         let parts_file_path = tmp_dir.join(BLUEPRINT_FILENAME);
...
//         }
//
//         fn dummy_upload_manifest(&self) -> Result<UploadSlots> {
//             Ok(UploadSlots {
//                 slots: vec![
//                     Slot {
//                         key: "KeyA".to_string(),
...
//         let test_env = setup_test_env()?;
//         let manifest = UploadSlots { slots: vec![] };
